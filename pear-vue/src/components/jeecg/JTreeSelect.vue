  ...
  export default {
    ...
    methods: {
      ...
      filterTreeNode(input,treeNode){  
        //加载拼音组件-拼音搜索即适应中文也适应拼音
        const PinyinMatch = require('pinyin-match');
        let p_m = PinyinMatch.match(treeNode.data.props.title.toLowerCase(),input.toLowerCase())
        //console.log(p_m,'p_m')
        //匹配就返回true
        if(p_m){
          return true
        }else{
          return treeNode.data.props.title.toLowerCase().toLowerCase().indexOf(input.toLowerCase()) >= 0
        }
      },
      getCurrTreeData(){
